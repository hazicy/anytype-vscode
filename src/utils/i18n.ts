...
export class I18n {
  ...
  static initialize(context: vscode.ExtensionContext): void {
    ...
    const bundlePath = this.getBundlePath(context.extensionUri.fsPath, this.currentLocale);

    try {
      const bundleContent = fs.readFileSync(bundlePath, 'utf-8');
      this.bundle = JSON.parse(bundleContent);
    } catch (error) {
      console.warn(`Failed to load localization bundle from ${bundlePath}`, error);
      this.bundle = {};
    }
  }
  ...
  private static getBundlePath(extensionPath: string, locale: string): string {
    const filename = locale === SUPPORTED_LOCALES.ZH_CN
 ...
 */
export const localize = (key: string, ...args: (string | number)[]) => I18n.localize(key, ...args);
